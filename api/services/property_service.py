...
class PropertyService:
    ...
    async def create_property(
        ...
        try:
            ...
            existing = self.supabase.table('properties').select('id').eq(
                'organization_id', str(data.organization_id)
            ).eq('address', data.address).eq(
                'deleted_at', None
            ).execute()
            ...
            if 'details' in property_dict:
                property_dict['details'] = property_dict['details'].dict(exclude_none=True)
            ...
            # Create property
            result = self.supabase.table('properties').insert(
                property_dict
            ).execute()
    ...
    async def get_properties(
        ...
        try:
            ...
            if filters.min_square_footage:
                query = query.gte('details->>square_footage', filters.min_square_footage)
            
            if filters.max_square_footage:
                query = query.lte('details->>square_footage', filters.max_square_footage)
            ...
            # Apply group filter
            if filters.group_id:
                group_properties = self.supabase.table('property_group_members').select(
                    'property_id'
                ).eq('group_id', str(filters.group_id)).execute()
    ...
    async def update_property(
        ...
        try:
            ...
            # Update property
            result = self.supabase.table('properties').update(
                update_dict
    ...
    async def delete_property(
        ...
        try:
            if hard_delete:
                # Permanent deletion
                result = self.supabase.table('properties').delete().eq(
                    'id', str(property_id)
                ).execute()
            else:
                # Soft delete
    ...
    async def bulk_create_properties(
        ...
        for idx, property_data in enumerate(data.properties):
            try:
                ...
                failed += 1
                if not data.skip_duplicates or e.status_code != status.HTTP_409_CONFLICT:
                    errors.append({
                        'index': idx,
    ...
    async def create_property_group(
        ...
        try:
            ...
            existing = self.supabase.table('property_groups').select('id').eq(
                'organization_id', str(data.organization_id)
            ...
            result = self.supabase.table('property_groups').insert(
                group_data
            ).execute()
            ...
            if data.property_ids:
                await self.add_properties_to_group(
                    group.id,
                    data.property_ids,
                    user_id
                )
    ...
    async def add_properties_to_group(
        ...
        try:
            ...
            result = self.supabase.table('property_group_members').upsert(
                members
            ).execute()
