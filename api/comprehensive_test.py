    ...
    try:
        ...
        for table in tables:
            try:
                ...
                table_results[table] = f"SUCCESS: {len(result.data) if result.data else 0} records"
            except Exception as e:
...
def test_api_endpoints():
    ...
    try:
        response = requests.post(f"{API_BASE}/api/auth/register", json=test_data, timeout=15)
        if response.status_code == 200:
            print("SUCCESS: Registration endpoint working")
            endpoint_working = True
        elif response.status_code == 400 and "Invalid API key" in response.text:
            print("PARTIAL: Registration endpoint responding but using wrong Supabase project")
            endpoint_working = False
        else:
            print(f"FAILED: Registration endpoint error - {response.status_code} - {response.text}")
            endpoint_working = False
    except Exception as e:
    ...
    try:
        reset_response = requests.post(f"{API_BASE}/api/auth/reset-password", 
                                     json={"email": "test@example.com"}, timeout=10)
        if reset_response.status_code == 200:
            print("SUCCESS: Password reset endpoint working")
        else:
            print(f"WARNING: Password reset endpoint issue - {reset_response.status_code}")
    except Exception as e:
...
def main():
    ...
    # Overall assessment
    core_working = results["direct_supabase"][0] if isinstance(results["direct_supabase"], tuple) else results["direct_supabase"]
