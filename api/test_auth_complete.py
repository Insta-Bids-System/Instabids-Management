...
def test_database_tables():
    ...
    try:
        ...
        print(f"user_profiles table: {len(profiles.data) if profiles.data else 0} records")
    ...
    for method, endpoint, data in endpoints:
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                print(f"FAILED: {method} {endpoint} - {response.status_code} - {response.text[:100]}")
...
def main():
    ...
    print("\n=== FINAL RESULTS ===")
    working_count = sum(1 for v in results.values() if v is True or (isinstance(v, int) and v > 0))
    
    for test_name, result in results.items():
        status = "PASS" if (result is True or (isinstance(result, int) and result > 0)) else "FAIL"
