...
from ..models.auth import (
    ...
    ResetPasswordRequest, NewPasswordRequest,
...
async def login(request: LoginRequest):
    ...
    try:
        ...
        profile_response = supabase_service.client.table("user_profiles").select("*").eq("id", user.id).single().execute()
...
async def refresh_token(request: RefreshTokenRequest):
    ...
    try:
        ...
        # Get user profile
        profile_response = supabase_service.client.table("user_profiles").select("*").eq("id", user_id).single().execute()        
        if not profile_response.data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="User not found"
...
async def reset_password(request: ResetPasswordRequest):
    ...
    try:
        ...
        logger.error(f"Password reset failed: {e}")        # Don't reveal if email exists or not
        return MessageResponse(
            success=True,
...
async def update_profile(
    ...
    try:
        ...
        if update_data:
            response = supabase_service.client.table("user_profiles").update(
                update_data
            ).eq("id", current_user.id).execute()
            
            if response.data:
                profile = response.data[0]
                return UserResponse(                    id=profile["id"],
                    email=profile["email"],
                    full_name=profile["full_name"],
                    user_type=profile["user_type"],
                    organization_id=profile["organization_id"],
