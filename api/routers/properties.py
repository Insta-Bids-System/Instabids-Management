...
async def export_properties(
    ...
    if format == 'csv':
        ...
        writer.writerow([
            ...
            'type', 'status', 'manager_id', 'square_footage',
            'year_built', 'units', 'amenities'
        ])
        ...
        json_data = json.dumps(
            [prop.dict() for prop in properties],
            default=str,
            indent=2
        )
        return StreamingResponse(
            ...
            headers={
                'Content-Disposition': 'attachment; filename="properties.json"'
            }
        )

# Property Groups

@router.post("/groups", response_model=PropertyGroup, status_code=status.HTTP_201_CREATED)
...
async def list_property_groups(
    ...
    # Get property counts
    for group in groups:
        count_result = supabase.table('property_group_members').select(
            'property_id', count='exact'
        ).eq('group_id', str(group.id)).execute()
        group.property_count = count_result.count or 0
    
    return PropertyGroupListResponse(
        groups=groups,
        total=len(groups)
    )
